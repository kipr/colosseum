...
- Persistence: `pg` against PostgreSQL 18. Local app and tests require Docker Compose (`DATABASE_URL` / `TEST_DATABASE_URL`). Production Cloud Run uses `CLOUD_SQL_CONNECTION_NAME` plus `DB_USER` / `DB_PASSWORD` / `DB_NAME`. The async `Database` adapter converts `?` placeholders to `$n`. Inserts that need the generated key include `RETURNING id`; `run()` exposes that as `lastID`.
