...
- Persistence: `pg` against PostgreSQL 18. Local app and tests require Docker Compose (`DATABASE_URL` / `TEST_DATABASE_URL`). Production Cloud Run uses `CLOUD_SQL_CONNECTION_NAME` plus `DB_USER` / `DB_PASSWORD` / `DB_NAME`. A shared async database adapter still normalizes SQL parameters (`?` → `$n`) until Phase 4.
