...
- Database access goes through the async `Database` abstraction in `src/server/database/connection.ts`. SQL generally uses `?` placeholders, converted by the PostgreSQL adapter. The running app always uses Postgres; keep leftover SQLite adapter tests working until Phase 3 schema collapse. Use adapter transactions for multi-step state changes.
- Schema is modular: each `src/server/database/schema/*.ts` module contributes a `SchemaModule`; register it in `schema/index.ts`. App init always runs `initializePostgres`. Dual `sqlite:` blocks remain until Phase 3.
