...
- Database access goes through the async `Database` abstraction in `src/server/database/connection.ts`. SQL generally uses `?` placeholders, converted by the PostgreSQL adapter. The running app and tests use Postgres only. Use adapter transactions for multi-step state changes.
