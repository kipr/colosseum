...
| `AuditLogEntry`, `AuditLogResponse`                                                                                                      | `audit.ts`               | **Fully typed.** `routes/audit.ts` imports both DTOs, defines an `AuditLogRow` interface that mirrors a single shared `SELECT` list (used by both GET handlers), and constructs each response via a `toAuditLogEntry` mapper + `typedJson`. The mapper normalises `created_at` to an ISO string so the wire shape is the same under SQLite and PostgreSQL.   |
...
3. `routes/awards.ts` — every handler. Type the template /
   ...
   via the service).
4. `routes/events.ts` — `GET /events/public` and
...
5. `routes/chat.ts` — delete the local `ChatMessage` interface,
