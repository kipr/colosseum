...
function parseTeamString(
  teamStr: string,
): { teamNumber: string; displayName: string } | null {
  ...
  for (let rowIdx = 0; rowIdx < values.length; rowIdx++) {
    ...
    for (let colIdx = 0; colIdx < row.length; colIdx++) {
      ...
      if (match) {
        ...
        for (
          let searchOffset = 1;
          searchOffset <= maxSearchRows &&
          rowIdx + searchOffset < values.length;
          searchOffset++
        ) {
          ...
          if (potentialTeam) {
            if (!team1) {
              ...
              !team2 &&
              potentialTeam.teamNumber !== team1.teamNumber
            ) {
              team2 = potentialTeam;
              team2Row = rowIdx + searchOffset;
        ...
        if (!team1 || !team2) {
          ...
          if (!team2) {
            ...
            for (
              ...
              searchRow < Math.min(rowIdx + maxSearchRows, values.length);
              searchRow++
            ) {
        ...
        const finalsWinnerCell =
          bracketSize === 64 ? get64TeamFinalsWinnerCell(gameNumber) : null;
        ...
        if (finalsWinnerCell) {
          ...
          if (
            checkRow >= 0 &&
            checkRow < values.length &&
            values[checkRow] &&
            values[checkRow][colIdx]
          ) {
          ...
          const searchDirection = getWinnerSearchDirection(
            bracketSize,
            gameNumber,
          );
          ...
          outerLoop: if (searchDirection === 'right') {
            // Search RIGHT (standard direction for most brackets)
            for (
              let checkCol = winnerSearchStartCol;
              checkCol < Math.min(winnerSearchStartCol + 10, 37);
              checkCol++
            ) {
            ...
            for (
              let checkCol = teamColumn - 1;
              checkCol >= Math.max(teamColumn - 10, 0);
              checkCol--
            ) {
              for (const checkRow of searchRows) {
                if (checkRow < 0 || checkRow >= values.length) continue;
...
function detectBracketSize(sheetName: string): number {
  ...
  if (normalized.startsWith('de 64') || normalized.startsWith('de64'))
    return 64;
  if (normalized.startsWith('de 32') || normalized.startsWith('de32'))
    return 32;
  if (normalized.startsWith('de 16') || normalized.startsWith('de16'))
    return 16;
 ...
 */
function getWinnerSearchDirection(
  bracketSize: number,
  gameNumber: number,
): 'left' | 'right' {
...
function getBracketLookupTable(size: number): Record<number, string> {
  switch (size) {
    case 4:
      return BRACKET_TARGET_CELLS_4_TEAM;
    case 8:
      return BRACKET_TARGET_CELLS_8_TEAM;
    case 16:
      return BRACKET_TARGET_CELLS_16_TEAM;
    case 32:
      return BRACKET_TARGET_CELLS_32_TEAM;
    case 64:
      return BRACKET_TARGET_CELLS_64_TEAM;
    default:
      return BRACKET_TARGET_CELLS_16_TEAM;
  }
}
...
export async function writeWinnerToBracket(
  ...
  for (
    let searchOffset = 1;
    searchOffset <= maxSearchRows && gameRow + searchOffset < values.length;
    searchOffset++
  ) {
    ...
    if (potentialTeam) {
      if (!team1Info) {
        ...
        !team2Info &&
        potentialTeam.teamNumber !== team1Info.teamNumber
      ) {
  ...
  for (const col of bracketColumns) {
    ...
    const searchEnd = Math.min(
      values.length - 1,
      (team2Row > 0 ? team2Row : team1Row) + 2,
    );

    // First, check if midRow itself is empty or has the winner
    if (midRow >= searchStart && midRow <= searchEnd) {
      const cellVal = values[midRow]
        ? String(values[midRow][col] || '').trim()
        : '';
      if (
        cellVal === '' ||
        cellVal === winnerDisplayName ||
  ...
  if (targetCol === -1) {
    ...
    targetRow = midRow;
    console.warn(
      `Could not find exact advancement cell for Game ${gameNumber}, using fallback: col ${targetCol} (${columnToLetter(targetCol)}), row ${targetRow}`,
    );
  }
...
export async function clearWinnerFromBracket(
  ...
  console.warn(
    `clearWinnerFromBracket: Game ${gameNumber} not in ${bracketSize}-team lookup table, cannot clear`,
  );
}
