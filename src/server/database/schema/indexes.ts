/**
 * Shared CREATE INDEX statements.
 *
 * The two dialects emit identical `CREATE INDEX IF NOT EXISTS …` SQL today,
 * so a single ordered list is applied verbatim from `init.ts`.
 */
...
export const ALL_INDEXES: readonly string[] = [
  ...
  // Bracket revert traversal (team source lookups)
  ...
  // Score submissions event-scoped
...
/**
 * Postgres-only indexes (e.g. on the connect-pg-simple session table).
 */
export const POSTGRES_ONLY_INDEXES: readonly string[] = [
