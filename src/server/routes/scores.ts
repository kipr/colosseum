...
router.get('/by-spreadsheet/:configId', requireAuth, async (req: AuthRequest, res: express.Response) => {
  try {
...
router.post('/:id/accept', requireAuth, async (req: AuthRequest, res: express.Response) => {
  try {
    const { id } = req.params;
    const db = await getDatabase();
    
    const score = await db.get('SELECT * FROM score_submissions WHERE id = ?', [id]);
    if (!score) {
    ...
    if (!config) {
      return res.status(400).json({ error: 'Spreadsheet configuration not found' });
    }
    ...
    try {
      if (isHeadToHead) {
        ...
        if (!gameNumber || !winnerTeamNumber || !winnerDisplay) {
          return res.status(400).json({ 
            error: 'Head-to-head score must have game number, winner team number, and winner display name' 
          });
        }
      ...
      res.status(500).json({ error: sheetError.message || 'Failed to submit to spreadsheet' });
    }
  } catch (error) {
...
// Reject a score
router.post('/:id/reject', requireAuth, async (req: AuthRequest, res: express.Response) => {
  try {
    const { id } = req.params;
    const db = await getDatabase();
...
router.post('/:id/revert', requireAuth, async (req: AuthRequest, res: express.Response) => {
  try {
    const { id } = req.params;
    const db = await getDatabase();
