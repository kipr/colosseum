    ...
    try {
      ...
      if (req.query.all === 'true') {
        configs = await db.all(
          'SELECT * FROM spreadsheet_configs WHERE is_active IS TRUE ORDER BY spreadsheet_name, sheet_name',
        );
      } else if (req.query.shared === 'true') {
        configs = await db.all(
          'SELECT * FROM spreadsheet_configs ORDER BY spreadsheet_name, sheet_name, created_at DESC',
        );
      } else {
        configs = await db.all(
      ...
      res
        .status(500)
        .json({ error: 'Failed to fetch spreadsheet configurations' });
    }
  },
);
...
router.get(
  '/spreadsheets/by-spreadsheet/:spreadsheetId/configs',
  requireAuth,
  async (req: AuthRequest, res: Response) => {
    try {
...
router.put(
  '/spreadsheets/by-spreadsheet/:spreadsheetId/deactivate',
  requireAuth,
  async (req: AuthRequest, res: Response) => {
    try {
      const { spreadsheetId } = req.params;
...
router.delete(
  '/spreadsheets/by-spreadsheet/:spreadsheetId',
  requireAuth,
  async (req: AuthRequest, res: Response) => {
    try {
      const { spreadsheetId } = req.params;
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (existing) {
        return res
          .status(400)
          .json({ error: 'This spreadsheet is already linked' });
      }
    ...
    try {
      ...
      if (!spreadsheetId || !sheetName || !sheetPurpose) {
        return res.status(400).json({
          error: 'Spreadsheet ID, sheet name, and purpose are required',
        });
      }
      ...
      if (existing) {
        return res
          .status(400)
          .json({ error: 'This sheet is already configured' });
      }
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (isAuthError) {
        res.status(401).json({
          error:
            'Your Google authentication has expired. Please log out and log back in.',
          needsReauth: true,
        });
      } else {
        res
          .status(500)
          .json({ error: 'Failed to get sheets from spreadsheet' });
      }
    }
  },
);
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (isAuthError) {
        ...
        res
          .status(500)
          .json({ error: 'Failed to list drives from Google Drive' });
      }
    }
  },
);
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (isAuthError) {
        ...
        res
          .status(500)
          .json({ error: 'Failed to list spreadsheets from Google Drive' });
      }
    }
  },
);
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (existing) {
        return res
          .status(400)
          .json({ error: 'This sheet configuration already exists' });
      }
...
router.put(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      await db.run(
        'UPDATE spreadsheet_configs SET is_active = true WHERE id = ?',
        [id],
      );
...
router.put(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      await db.run(
        'UPDATE spreadsheet_configs SET is_active = false WHERE id = ?',
        [id],
      );
...
router.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (deleteShared) {
        await db.run('DELETE FROM spreadsheet_configs WHERE id = ?', [id]);
      } else {
        await db.run(
      ...
      res
        .status(500)
        .json({ error: 'Failed to delete spreadsheet configuration' });
    }
  },
);
