...
import { seedEvent, seedAuditLog } from './helpers/seed';
...
describe('Audit Routes', () => {
  ...
  const authUser = { id: 1, is_admin: true };
  ...
  describe('Authentication Boundaries', () => {
    it('GET /audit/event/:eventId returns 403 when not authenticated', async () => {
    ...
    it('GET /audit/entity/:type/:id returns 403 when not authenticated', async () => {
      const app = createTestApp();
      app.use('/audit', auditRoutes);
      const unauthServer = await startServer(app);
    ...
    it('returns 403 when authenticated but not admin', async () => {
      ...
      try {
        const res = await http.get(
          `${nonAdminServer.baseUrl}/audit/event/${event.id}`,
        );
        expect(res.status).toBe(403);
      } finally {
  ...
  it('POST /audit is not exposed', async () => {
